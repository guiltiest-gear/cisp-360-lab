...
// Pick the winner
void winner(int comp_choice, int user_choice,
            bool &play_again) { // play_again must be passed by reference
  cout << '\n';
  if (comp_choice == rock) {
    switch (user_choice) {
    case rock:
    ...
    case scissors:
      ...
      break;
             ...
             scissors) { // Although not needed, improves code clarity
    switch (user_choice) {
    case rock:
      cout << "Player wins!";
      play_again = false;
...
int main() {
  // For some god awful reason, g++ won't shut up about user_number being
  // uninitialized The code will work exactly the same, and the 0 will get
  // overritten anyway I genuinely have no clue why it does this, but I don't
  // have the energy to care At least doing this gets g++ to put a sock in it
