...
void player(char array[][rows_and_columns], char letter) {
  ...
  // The program will literally work the extact same way without these being set
  // to a value The 0's will get overwritten anyway I genuinely have no clue why
  // it keeps bugging me about this
  int row = 0, column = 0;
  bool occupied = false;
  ...
  do {
    ...
    if (array[(row - 1)][(column - 1)] == '*') {
      ...
      occupied ==
      false); // Make sure players are putting their X's and O's in empty spots
}

// Check for horizontal win
bool horizontal_win(const char array[][rows_and_columns], const char letter,
                    string &game_winner) {
  int across = 0;
  bool winner = false;
...
bool vertical_win(const char array[][rows_and_columns], const char letter,
                  string &game_winner) {
  int down = 0;
  bool winner = false;
...
bool diagonal_win_left_to_right(const char array[][rows_and_columns],
                                const char letter, string &game_winner) {
  int diagonal = 0;
  bool winner = false;
...
bool diagonal_win_right_to_left(const char array[][rows_and_columns],
                                const char letter, string &game_winner) {
  int diagonal = 0;
  bool winner;

  for (int row = 0, column = (rows_and_columns - 1); row < rows_and_columns;
       row++, column--) {
    if (array[row][column] == letter) {
      diagonal++;
    }
...
// Did someone win yet?
int winner_ultimate_check(const char array[][rows_and_columns],
                          const char letter, string &game_winner) {
  bool winner = false;
