...
// Description: Write a program that displays a menu allowing the user to select
// one of these four gases. After a
//              selection has been made, the user should enter the number of
//              seconds the sound to travel in this medium from its source to
//              the location at which it was detected. The program should then
//              report how far away (in meters) the source of the sound was from
...
// Ask the user the number of seconds it took the sound to travel to the
// detection location
void time_of_detection() {
  cout
      << "How much time passed until the detection of the sound (in seconds): ";
  cin >> seconds;
  // Detect if the time is less than 0 seconds or greater than 30 seconds
  if (seconds < 0.0 || seconds > 30.0) {
    cout << "Error: Times below 0 seconds or times above 30 seconds are not "
            "valid!"
         << '\n';
    exit(1);
  }
...
void distance() {
  ...
  switch (choice) {
  default:
    cout << "Error: Something really bad happened, this message physically "
            "should not be able to display!"
         << '\n';
    exit(1);
  case 1:
